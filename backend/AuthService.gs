/**
 * Authentication: salted iterative SHA-256 hashing, server-side sessions,
 * role checks, and branch isolation enforced on every protected operation.
 */
...
var Auth = (function () {
  ...
  function digestHex(str) {
    return bytesToHex(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, str));
  }
  ...
  function makeSalt() {
    return digestHex(Utilities.getUuid() + ':' + Date.now() + ':' + Math.random()).slice(0, 24);
  }
  ...
  function newToken() {
    return digestHex(Utilities.getUuid() + Date.now() + Math.random());
  }

  var sessionsRepo = function () { return SheetsRepo.repo('Sessions'); };
  ...
  function createSession_(user) {
    ...
    repo.insert({
      ...
      created_at: nowIso(),
      expires_at: Utilities.formatDate(expires, CONFIG.timezone(), "yyyy-MM-dd'T'HH:mm:ss"),
      active: true
    });
