/**
 * Setup / demo seed port of Setup.gs, plus local-only backup helpers
 * (export / import / reset) that only exist in the browser backend.
 */
import { SheetsRepo, SHEET_HEADERS, Ids, loadDB, wipeDB, replaceDB } from './store.js';
import { Auth } from './auth.js';
import { Activity } from './activity.js';
import { fail, nowIso, ORDER_STATUS, USER_STATUS, USER_ROLES } from './constants.js';
...
export function sheetsAreReady_() {
  var db = loadDB();
  return Object.keys(SHEET_HEADERS).every(function (name) { return !!db.sheets[name]; });
}

export function ensureSetup_() {
...
export function createFirstAdmin(username, password, fullName) {
  ensureSetup_();
...
function seedOrder(orderItems, orders, branchId_, by, status, lines) {
  var id = Ids.orderId();
  var now = nowIso();
  var rec = orders.insert({
    order_id: id, order_number: id, branch_id: branchId_, created_by: by, status: status,
...
export function loadDemoData() {
  ...
  for (var itemIdx = 1; itemIdx <= demoItems.length; itemIdx++) {
    var iid = 'ITM-' + String(itemIdx);
    [b1, b2, b3].forEach(function (bid) {
      branchItems.insert({
        branch_id: bid, item_id: iid, is_available: true, max_quantity: '',
        updated_at: nowIso()
      });
    });
  }
